...
namespace AmberGL::SoftwareRenderer::RenderObject
{
	...
	{
		static_assert(std::is_same_v<T, RGBA8> || std::is_same_v<T, Depth> || std::is_same_v<T, Stencil>, "FrameBuffer supports RGBA8 (Color), Depth (float), or Stencil (uint8_t).");
	...
	public:
		...
		void Clear() const
		{
			...
			else if constexpr (std::is_same<T, Stencil>::value)
			{
				SSEClear(Data, Size, 0);
			}
		}
		...
		void SetPixel(int p_x, int p_y, const T& p_value) const
		{
			...
			{
				Data[p_y * Width + p_x] = static_cast<uint32_t>(p_value);
			}
		}
		...
		T GetPixel(uint32_t p_x, uint32_t p_y) const
		{
			...
			{
				return static_cast<uint8_t>(Data[p_y * Width + p_x] & 0xFF);
			}
		...
		void UpdateAttachedTextureObject(TextureObject* p_textureObject)
		{
			...
			else if constexpr (std::is_same<T, RGBA8>::value)
			{
				for (uint32_t y = 0; y < Height; y++)
				{
					for (uint32_t x = 0; x < Width; x++)
			...
			else if constexpr (std::is_same<T, Stencil>::value)
			{
				for (uint32_t y = 0; y < Height; y++)
				{
					...
					{
						uint32_t index = y * Width + x;
						uint8_t s = static_cast<uint8_t>(Data[y * Width + x] & 0xFF);

						textureData[index * 4 + 0] = s;
						textureData[index * 4 + 1] = s;
						textureData[index * 4 + 2] = s;
						textureData[index * 4 + 3] = 255;
					}
				}
			}

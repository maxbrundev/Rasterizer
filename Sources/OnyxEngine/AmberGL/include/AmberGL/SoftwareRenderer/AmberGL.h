...
namespace AmberGL
{
	namespace SoftwareRenderer::Programs
	{
		class AProgram;
	}
	...
	API_AMBERGL void GetViewport(uint16_t* p_x, uint16_t* p_y, uint16_t* p_width, uint16_t* p_height);
	...
	API_AMBERGL uint32_t GetActiveFrameBufferWidth();
	API_AMBERGL uint32_t GetActiveFrameBufferHeight();
	API_AMBERGL void ResizeFrameBuffer(uint32_t p_frameBuffer, uint16_t p_width, uint16_t p_height);

	API_AMBERGL uint32_t GetDepthBufferRowSize();

	API_AMBERGL void CopyTexImage2D(uint32_t p_target, uint32_t p_level, uint32_t p_internalFormat, int p_x, int p_y, uint32_t p_width, uint32_t p_height, uint32_t p_border);

	API_AMBERGL void SwapBuffers();
	API_AMBERGL uint32_t* GetFrontBufferData();
	API_AMBERGL uint32_t GetFrontBufferRowSize();

	API_AMBERGL bool HasActiveColorBuffer();
	API_AMBERGL bool HasActiveDepthBuffer();
	...
	API_AMBERGL void GenUniformBuffers(uint32_t p_count, uint32_t* p_buffers);
	API_AMBERGL void DeleteUniformBuffers(uint32_t p_count, const uint32_t* p_buffers);
	API_AMBERGL void BindUniformBuffer(uint32_t p_buffer);
	API_AMBERGL void UniformBufferData(size_t p_size, const void* p_data, uint16_t p_usage);
	API_AMBERGL void UniformBufferSubData(size_t p_offset, size_t p_size, const void* p_data);
	API_AMBERGL void BindUniformBufferBase(uint32_t p_bindingPoint, uint32_t p_buffer);
	API_AMBERGL bool GetUBOData(uint32_t p_bindingPoint, size_t p_offset, size_t p_size, void* p_data);

	API_AMBERGL uint32_t GetBoundUBO(uint32_t bindingPoint);

	API_AMBERGL size_t GetUBOSize(uint32_t uboID);

	API_AMBERGL void StencilFunc(uint16_t p_func, int p_ref, uint32_t p_mask);
	API_AMBERGL void StencilOp(uint16_t p_sfail, uint16_t p_dpfail, uint16_t p_dppass);
	API_AMBERGL void StencilMask(uint32_t p_mask);
	API_AMBERGL bool HasActiveStencilBuffer();

	API_AMBERGL void ResizeDefaultFramebuffer(uint16_t p_width, uint16_t p_height);
}

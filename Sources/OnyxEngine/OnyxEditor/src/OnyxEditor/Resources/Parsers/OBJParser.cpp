...
bool OnyxEditor::Resources::Parsers::OBJParser::LoadOBJ(const std::string& p_filePath, std::vector<Mesh*>& p_meshes, std::vector<MaterialData>& p_materialsData)
{
...
bool OnyxEditor::Resources::Parsers::OBJParser::ParseFile(const std::string& p_filePath, std::vector<Mesh*>& p_meshes, std::vector<MaterialData>& p_materialsData)
{
	...
	while (std::getline(parser, line))
	{
		...
		if (line.substr(0, 7) == "mtllib ")
		{
			...
			for (auto& materialData : p_materialsData)
			{
				...
				{
					materialData.MetallicRoughnessTexturePath = m_directory + materialData.MetallicRoughnessTexturePath;
				}

				if (!materialData.NormalTexturePath.empty())
				{
					materialData.NormalTexturePath = m_directory + materialData.NormalTexturePath;
				}

				if (!materialData.EmissiveTexturePath.empty())
				{
					materialData.EmissiveTexturePath = m_directory + materialData.EmissiveTexturePath;
				}

				if (!materialData.AOTexturePath.empty())
				{
					materialData.AOTexturePath = m_directory + materialData.AOTexturePath;
				}

				if (!materialData.DisplacementTexturePath.empty())
				{
					materialData.DisplacementTexturePath = m_directory + materialData.DisplacementTexturePath;
				}
			}
		}
	...
	{
		std::vector<AmberGL::Geometry::Vertex>& vertices = data.Vertices;
		...
		for (size_t i = 0; i < indices.size(); i += 3)
		{
			...
			AmberGL::Geometry::Vertex& v0 = vertices[i0];
			AmberGL::Geometry::Vertex& v1 = vertices[i1];
			AmberGL::Geometry::Vertex& v2 = vertices[i2];
...
void OnyxEditor::Resources::Parsers::OBJParser::ParseIndices(const std::string_view& p_line, std::vector<std::tuple<uint32_t, uint32_t, uint32_t>>& p_indices)
{
	m_indicesBuffer.clear();

	std::stringstream ss(p_line.substr(2).data());
	std::string token;
	...
	while (ss >> token)
	{
		...
		else if (sscanf_s(token.c_str(), "%d//%d", &v, &vt, &vn) == 2)
		{
			v--;
			vn--;
		}
		else if (sscanf_s(token.c_str(), "%d", &v, &vt, &vn) == 1)
		{
			v--;
		}

		m_indicesBuffer.emplace_back(v, vt, vn);
	}

	for (int i = 1; i + 1 < m_indicesBuffer.size(); i++)
	{
		p_indices.emplace_back(m_indicesBuffer[0]);
		p_indices.emplace_back(m_indicesBuffer[i]);
		p_indices.emplace_back(m_indicesBuffer[i + 1]);
	}
}
